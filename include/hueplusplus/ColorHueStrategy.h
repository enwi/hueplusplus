...
public:
    ...
    virtual bool setColorHueSaturation(const HueSaturation& hueSat, uint8_t transition, HueLight& light) const = 0;
    ...
    virtual bool setColorXY(const XYBrightness& xy, uint8_t transition, HueLight& light) const = 0;
    ...
    //! \param light A reference of the light
    virtual bool alertHueSaturation(const HueSaturation& hueSat, HueLight& light) const = 0;
    //! \brief Virtual function that lets the light perform one breath cycle in
    //! the specified color.
    //!
    //! \param xy The color in XY and brightness
    //! \param light A reference of the light
    virtual bool alertXY(const XYBrightness& xy, HueLight& light) const = 0;
    ...
    //! \param light A reference of the light
    virtual HueSaturation getColorHueSaturation(HueLight& light) const = 0;
    //! \brief Virtual function that returns the current color of the light as hue
    //! and saturation
    //!
    //! \note This should not update the lights state
    //! \param light A const reference of the light
    virtual HueSaturation getColorHueSaturation(const HueLight& light) const = 0;
    //! \brief Virtual function that returns the current color of the light as xy
    //!
    //! Should update the lights state by calling refreshState()
    //! \param light A reference of the light
    //! \return XY and brightness of current color
    virtual XYBrightness getColorXY(HueLight& light) const = 0;
    //! \brief Virtual function that returns the current color of the light as xy
    //!
    //! \note This should not update the lights state
    //! \param light A const reference of the light
    //! \return XY and brightness of current color
    virtual XYBrightness getColorXY(const HueLight& light) const = 0;
    //! \brief Virtual dtor
